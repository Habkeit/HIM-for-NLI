...
from data import NLIDataset, collate_fn
from model import HIM
...
def main(train_file,
    ...
    with open(train_file, "rb") as pkl:
        train_data_raw = pickle.load(pkl)
    train_data = NLIDataset(train_data_raw)

    train_loader = DataLoader(train_data, shuffle=True, batch_size=batch_size, collate_fn=collate_fn)

    print("\t* Loading validation data...")
    with open(valid_file, "rb") as pkl:
        valid_data_raw = pickle.load(pkl)
    valid_data = NLIDataset(valid_data_raw)
    ...
    print("\n",
          20 * "=",
          "Training HIM model on device: {}".format(device),
          20 * "=")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Train the HIM model on SNLI")
